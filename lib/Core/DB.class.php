...
class Core_DB
{
	...
	public function __construct($db = DB_MAIN)
	{
		...
		if ($DATABASES [$db] ['persistent'])
		{
			$this->_db = mysql_pconnect ( $DATABASES [$db] ['host'], $DATABASES [$db] ['username'], $DATABASES [$db] ['password'] );
		}
		else
		{
			$this->_db = mysql_connect ( $DATABASES [$db] ['host'], $DATABASES [$db] ['username'], $DATABASES [$db] ['password'] );
		}
		mysql_select_db ( $DATABASES [$db] ['db'], $this->_db );
	...
	private function _createSqlUpdate($table, $data, $index)
	{
		...
		foreach ( $data as $key => $value )
		{
			is_string ( $value ) ? $query .= "`$key` = '" . Core_Helper::sanitize ( $value ) . "', " : $query .= "`$key` = $value, ";
		}
		$query = substr ( $query, 0, strlen ( $query ) - 2 );
	...
	/**
	 * Lists all Databases accessible by the account
	 *
	 * @return array
	 */
	public function listDBs()
	{
		$result = mysql_listdbs ( $this->_db );
		
		$returnvalue = array ();
		$data = mysql_fetch_array ( $result );
		while ( $data )
		{
			$returnvalue [] = $data ['Database'];
			$data = mysql_fetch_array ( $result );
		}
